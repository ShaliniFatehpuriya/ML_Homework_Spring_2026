...
class KernelPCA:
    ...
    def _kernel(self, X, Y):
        if self.kernel == "rbf":
            ...
            raise ValueError("Use legal kernal")
    def fit(self, X):
        self.X = X
        K = self._kernel(X, X)
        Kc = center_kernel(K)
...
class KernelLDA:
    ...
    def _kernel(self, X, Y):
        if self.kernel == "rbf":
            ...
            raise ValueError("not leagal arg")
    def fit(self, X, y):
        self.X = X
        K = self._kernel(X, X)
        K = center_kernel(K)
...
def show_reconstructions(pca, lda, X, save_path="./YaleDataConstructedOP/reconstruction_faces.png"):
...
def main():
    ...
    show_face_grid(pca.components, "Eigenfaces (PCA)", "./YaleDataConstructedOP/eigenfaces_grid.png")
    show_face_grid(fisherfaces, "Fisherfaces (LDA)", "./YaleDataConstructedOP/fisherfaces_grid.png")
